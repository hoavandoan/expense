...
import { Avatar, Button, Card, PressableFeedback, Spinner, useThemeColor } from 'heroui-native';
  ...
  return (
    <View className="flex-1 bg-background">
      ...
      <ScreenScrollView contentContainerStyle={{ padding: 20 }}>
        {/* Invite Code Card */}
        <View className="mb-6">
          <AppText className="text-[12px] font-bold text-muted uppercase tracking-widest mb-4 ml-1">MÃ MỜI</AppText>
          <Card className="rounded-2xl border border-accent/20 bg-accent/5 p-4">
            <View className="flex-row items-center justify-between">
              <View className="flex-1">
                <AppText className="text-muted text-xs mb-1">Chia sẻ mã này để mời bạn bè</AppText>
                <AppText className="text-2xl font-bold text-accent tracking-widest">{inviteCode || '------'}</AppText>
              </View>
              ...
              return (
                <View key={member.id}>
                  <View className="p-4 flex-row items-center">
                    <Avatar size="md" alt={userData?.name || 'User'} className="mr-4">
                      {userData?.avatar_url ? (
                        <Avatar.Image source={{ uri: userData.avatar_url }} asChild>
                          <Image source={{ uri: userData.avatar_url }} style={{ width: '100%', height: '100%' }} />
                        </Avatar.Image>
                      ) : (
                        <Avatar.Fallback className="bg-accent/10">
                          <AppText className="font-bold text-accent">{userData?.name?.charAt(0) || '?'}</AppText>
                        </Avatar.Fallback>
                      )}
                    </Avatar>
                    <View className="flex-1">
                      <AppText className="font-bold text-base">{userData?.name || 'Thành viên'}</AppText>
                      <AppText className="text-muted text-xs">{userData?.email}</AppText>
                    </View>
                    {isOwner && (
                      <View className="bg-accent/10 px-3 py-1 rounded-full">
                        <AppText className="text-accent text-[10px] font-bold">Chủ nhóm</AppText>
                      </View>
                    )}
                  </View>
                  {idx < members.length - 1 && <View className="h-px bg-divider/10 mx-4" />}
                </View>
              );
            })}
          </Card>
        ...
        >
          <View className="flex-row items-center gap-2">
            <IconSymbol name="square.and.arrow.up" size={18} color={foreground} />

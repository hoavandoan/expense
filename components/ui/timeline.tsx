...
const InternalTimelineItem = ({
  ...
  const iconRotate = useSharedValue(1);

  const prevStatus = React.useRef(item.status);

  useEffect(() => {
    const statusChanged = prevStatus.current !== item.status;
    const wasComplete = prevStatus.current === 'complete';
...
InternalTimelineItem.displayName = 'InternalTimelineItem';
